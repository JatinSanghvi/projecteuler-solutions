...
def maximum_path_sum(triangle):
    path_sums = [[0] * (i + 2) for i in range(len(triangle) + 1)]

    for row_num in range(len(triangle)):
        for cell_num in range(len(triangle[row_num])):
            path_sums[row_num + 1][cell_num + 1] =\
                max(path_sums[row_num][cell_num], path_sums[row_num][cell_num + 1]) + triangle[row_num][cell_num]
    return max(path_sums[len(triangle)])


print maximum_path_sum(test_triangle)  # test
print maximum_path_sum(problem_triangle)  # solution
